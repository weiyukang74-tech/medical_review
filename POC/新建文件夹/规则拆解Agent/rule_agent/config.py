...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        raw_enable_thinking = os.getenv("MODEL_ENABLE_THINKING", "").strip().lower()
        if raw_enable_thinking in {"true", "1", "yes"}:
            enable_thinking: bool | None = True
        elif raw_enable_thinking in {"false", "0", "no"}:
            enable_thinking = False
        elif raw_enable_thinking == "":
            enable_thinking = None
        else:
            raise ValueError("MODEL_ENABLE_THINKING 只能是 true、false 或留空")
        settings = cls(
