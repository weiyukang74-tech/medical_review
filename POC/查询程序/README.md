...
3. 检验报告按查询规划中的关键词匹配 `itemName` 或 `indexName`，返回检验结果、单位、参考范围和结果状态等字段；
4. 患者信息只返回标签对应的字段；
5. 病历按 `medinsId + mdtrtId + recordName/recordType` 查询，并完整返回视图中的 `recordContent` 原文；复核上下文只在相同 `recordName` 的病历组内提取公共内容，公共内容按字段拼接展示，每条记录的差异内容也按字段拼接展示；
6. 首先执行主要来源；
7. 主要来源无数据、视图不可用或预期字段缺失时，自动查询次要来源；
8. 查询程序不判断医学事实和申诉命题。
...
如果存在对应规划，费用明细将按 `chrg_type` 和名称检索词过滤，检验报告将按 `itemName` 或 `indexName` 任一字段和关键词组过滤，不拼接两个字段。检验名称兼容“特异抗体/特异性抗体”等常见变体。病例文书不使用关键词筛选，查询结果保留完整原始记录；复核上下文只对相同 `recordName` 的病历进行公共内容提取。命中行不去重、不聚合；检验结果按证据所需字段返回。
